...
class Generic_WSI_Classification_Dataset(Dataset):
	def __init__(self,
		...
		label_cols = slide_data.columns.tolist()
		self.label_cols = label_cols
		self.label_cols.remove('slide_id')
		self.label_cols.remove('case_id')
	...
	def df_prep(data, label_cols):
		if label_cols[0] != 'label':
			data['label'] = data[label_cols].copy()
	...
	def test_split_gen(self, return_descriptor=False):
		if return_descriptor:
			index = [list(self.label_dicts.keys())[list(self.label_dicts.values()).index(i)] for i in range(self.num_classes)]
		...
		for split_name, ids in zip(['train', 'val', 'test'], [self.train_ids, self.val_ids, self.test_ids]):
			...
			for u in range(len(unique)):
				...
				if return_descriptor:
					df.loc[index[u], split_name] = counts[u]
...
class Generic_MIL_Dataset(Generic_WSI_Classification_Dataset):
	...
	def __getitem__(self, idx):
		...
		other_feature = []
		for i in range(1, len(self.label_cols)):
			other_feature.append(self.slide_data[self.label_cols[i]][idx])
		...
		if not self.use_h5:
			...
			return features, label, other_feature
		else:
			...
			return features, label,other_feature, coords
