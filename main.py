...
def main(args):
    ...
    for i in folds:
        ...
        print('training: {}, validing: {} testing: {}'.format(len(train_dataset), len(val_dataset),len(test_dataset)))
        datasets = (train_dataset,val_dataset, test_dataset)
        results, test_auc, test_acc, result_list,final_loss = train(datasets, i, args)
        ...
        final_loss.to_csv(os.path.join(args.results_dir,'train_loss_{}.csv'.format(i)))
    final_df = pd.DataFrame({'folds': folds, 'test_auc': all_test_auc, 'test_acc': all_test_acc })
    result_df = pd.DataFrame(result_df).T
    result_df.columns = ["Accuracy","Precision","Recall","F1-score","AUC"]
    result_df.index = ["Fold0","Fold1","Fold2","Fold3","Fold4"]
    ...
    result_df.to_csv(os.path.join(args.results_dir, 'Indicators_of_evaluation.csv'))

# Generic training settings
parser = argparse.ArgumentParser(description='Configurations for WSI Training')
parser.add_argument('--data_root_dir', type=str, default=None, help='feature data directory')
...
parser.add_argument('--seed', type=int, default=5, help='random seed for reproducible experiment (default: 1)')
...
if not os.path.isdir(args.results_dir):
    os.mkdir(args.results_dir)
args.results_dir = os.path.join(args.results_dir, str(args.exp_code) + '_{}fold_result'.format(args.seed))
if not os.path.isdir(args.results_dir):
    os.mkdir(args.results_dir)
