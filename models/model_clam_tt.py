...
class CLAM_SB(nn.Module):
    def __init__(self, gate = True, size_arg = "big", dropout = False, k_sample=8, n_classes=2, other_feature=True,
        ...
        if other_feature:
            fc_ = [nn.Linear(17, 128), nn.ReLU()]
            if dropout:
                fc_.append(nn.Dropout(0.25))
            self.other_feature = nn.Sequential(*fc_)
            self.classifiers = nn.Linear(size[1] + 128, n_classes)
        else:
    ...
    def forward(self, h, omis_feature, label=None, instance_eval=False, return_features=False, attention_only=False):
        device = h.device
...
class CLAM_MB(CLAM_SB):
    def __init__(self, gate = True, size_arg = "big", dropout = False, k_sample=8, n_classes=2, other_feature=False,
                 instance_loss_fn=nn.CrossEntropyLoss(), subtyping=False):
        super(CLAM_SB, self).__init__()
        self.size_dict = {"small": [256, 256, 128], "big": [1024, 512, 256]}
        size = self.size_dict[size_arg]
