...
def infer_single_slide(model, features, label, reverse_label_dict, k=1):
	...
	with torch.no_grad():
		if isinstance(model, (CLAM_SB,)):
			omis_feature = None # by lv.
			model_results_dict = model(features,omis_feature) # by lv.
			logits, Y_prob, Y_hat,A = model_results_dict['logits'], model_results_dict['Y_prob'], model_results_dict['Y_hat'],model_results_dict['A']
			Y_hat = Y_hat.item()
...
if __name__ == '__main__':
	...
	def_seg_params = {'seg_level': -1, 'sthresh': 15, 'mthresh': 11, 'close': 2, 'use_otsu': False, 'keep_ids': 'none', 'exclude_ids':'none'}
	...
	for i in range(len(process_stack)):
		...
		for key, val in filter_params.items():
			print('filter_params{}: {}'.format(key, val))
